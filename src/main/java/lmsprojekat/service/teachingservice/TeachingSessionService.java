...
public class TeachingSessionService extends AbstractCrudService<TeachingSessionDTO, TeachingSession, Long> {
    ...
    public TeachingSessionDTO toDTO(TeachingSession entity) {
        ...
        return new TeachingSessionDTO(
            ...
            entity.getTeachingType() != null
                ? new TeachingTypeDTO(
                    entity.getTeachingType().getId(),
                    entity.getTeachingType().getName(),
                    null,
                    null
                  )
                : null,
            entity.getLearningOutcomes() != null
                ? entity.getLearningOutcomes().stream()
    ...
    public TeachingSession toEntity(TeachingSessionDTO dto) {
        ...
        if (dto.getCourseRealizationId() != null) {
            entity.setCourseRealization(courseRealizationRepository.findById(dto.getCourseRealizationId())
                .orElseThrow(() -> new EntityNotFoundException("CourseRealization not found with id: " + dto.getCourseRealizationId())));
        }

        if (dto.getTeachingType() != null && dto.getTeachingType().getId() != null) {
            entity.setTeachingType(teachingTypeRepository.findById(dto.getTeachingType().getId())
                .orElseThrow(() -> new EntityNotFoundException("TeachingType not found with id: " + dto.getTeachingType().getId())));
        }
        ...
        if (dto.getLearningOutcomeIds() != null) {
            List<LearningOutcome> outcomes = dto.getLearningOutcomeIds().stream()
                .map(id -> learningOutcomeRepository.findById(id)
                    .orElseThrow(() -> new EntityNotFoundException("LearningOutcome not found with id: " + id)))
                .collect(Collectors.toList());
    ...
    protected void updateEntity(TeachingSession entity, TeachingSessionDTO dto) {
        ...
        if (dto.getTeachingType() != null && dto.getTeachingType().getId() != null) {
            TeachingType type = teachingTypeRepository.findById(dto.getTeachingType().getId())
                .orElseThrow(() -> new EntityNotFoundException("TeachingType not found with id: " + dto.getTeachingType().getId()));
