...
public interface StudentRepository extends SoftDeleteRepository<Student, Long> {
    ...
    @Query("""
      ...
      FROM TeacherOnCourse toc
        ...
        JOIN ca.student s
        JOIN StudentInYear siy ON siy.student.id = s.id
    ...
    @Query("""
      SELECT DISTINCT s
      FROM Student s
        LEFT JOIN StudentInYear siy ON siy.student = s
    ...
    @Query("""
      ...
      WHERE ca.student.id = :studentId AND ca.konacnaOcena IS NOT NULL
    """)
    Integer espbForPassed(@Param("studentId") Long studentId);
    ...
    @Query("""
      SELECT YEAR(siy.enrollmentDate)
